...
@Component({
  selector: 'app-teacher-practice-subjects-lessons',
  templateUrl: './teacher-practice-subjects-lessons.component.html',
  styleUrls: ['./teacher-practice-subjects-lessons.component.css']
})
export class TeacherPracticeSubjectsLessonsComponent implements OnInit {
  ...
  filterMethod() {
    ...
    .subscribe(params => {
      ...
      axios.post('http://localhost:3333/api/teacher-course-lessons', data).then((response) => {
        ...
        const lesson_modules = collect(response.data.lesson_modules).where('lesson_module_type', 1).toArray();
        let count = 0;
        let minutes = 0;
